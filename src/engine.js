/* CIPHER engine — hip hop & R&B banger builder for Suno v5 / v5.5.
...
const CipherEngine = (() => {
  // ---------------------------------------------------------------- profiles
  const PROFILES = {
    "v5.5": { id: "v5.5", styleCharLimit: 1000, lyricsCharLimit: 5000, hasSliders: true },
    v5: { id: "v5", styleCharLimit: 1000, lyricsCharLimit: 5000, hasSliders: true },
  };
  const DEFAULT_PROFILE = "v5.5";
  ...
  const OPTIONAL_BANS = ["airhorn", "dj tag", "risers", "disco", "live band", "saxophone", "brass", "choir"];
  ...
  const EDGES = {
    ...
    experimental: {
      ...
      note: "Experimental: high Weirdness for left-field choices, Style Influence still 65+ so the genre anchor survives. Generate in batches and keep the strangest take that still bangs.",
    },
  };
  ...
  function buildSections(ctx) {
    ...
    push("Outro", `hook motif repeats, hard stop${chordText(sh.outro)}`, "outro", { guide: "1–2 lines: the title one last time", harmony: sh.outro });
  ...
  function generate(input = {}) {
    ...
    return {
      ...
      blueprint,
      sliders: EDGES[edge].sliders,
      sliderNote: EDGES[edge].note,
      warnings,
      meta: {
