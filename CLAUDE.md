...
- **URP is live (2026-09-11).** `com.unity.render-pipelines.universal` 17.0.4; pipeline assets are created reproducibly by `Cipher.Game.Editor.UrpSetup.Configure` (menu **Cipher/Rendering/Configure URP**), not hand-clicked, so CI can rebuild them. Runtime materials use **`Exodus/InstancedLit`** (`Assets/Shaders/InstancedLit.shader`): instanced, per-instance colour, forward + shadow-caster + depth passes, ~192 variants.
- **THE SHADER VARIANT EXPLOSION IS FIXED, and the old explanation in this file was wrong.** The cause was never the Standard shader by itself: it was `m_InstancingStripping: 2` (Keep All) applied across a full PBR uber-shader. Under URP that setting made **URP Lit compile 589,824 variants** at roughly 16/sec, i.e. a build that never finishes. Two changes fixed it: set `m_InstancingStripping: 0` (Strip Unused) in `GraphicsSettings.asset`, and **remove any `Fallback` to URP Lit** from our shaders, because a fallback drags the whole uber-shader's variant set into the build. Runtime-created materials stay safe because `Exodus/InstancedLit` is in **Always Included Shaders**, which compiles all of its (few) variants. **Never set instancing stripping back to Keep All.**
